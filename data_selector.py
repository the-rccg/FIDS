...
def get_all_data(bricks_selected, axis_name_list, criteria_dict,
    ...
    - NumPy arrays are fixed-size, hence over-allocation & reduction creates copies too

    TODO:
    - Test memory & CPU for:  np.append vs. np.concat
    """
...
def reduce_cols(data, axis_name_list, selection=0):
    ...
    TODO: test other implementations
        ...
        Test 2: ValueError: could not convert string to float: 'PHAT J....' (since np.empty has 1 type rather than multiple)
        Test 3: selection = np.empty((len(data), len(axis_name_list)))
...
import operator
def slice_data(data, criteria_dict, axis_name_list=[], list_comp=True, ordered=[]):
    ...
    TODO:
    - Performance benchmark for Memory Usage and Speed
    - Boundary checks on memory to select algorithm
    """
    # Bulk slicing
    if list_comp:
        t1 = time.time()
